...
float micVolt = 0;      // variable to store the value read from the sensor pin
float micReading = 0;
float tempVolt = 0;      // variable to store the value read from the sensor pin
float tempReading = 0;
...
void loop() {
  ...
  //if (micVolt >= maxthreshold || micVolt <= minthreshold) {
//    micReading=0;
 // }else {
    micReading = (((micVolt >= 512) ? micVolt - 512 : 512 - micVolt ) + lastValueVoltMic + lastValueVoltMic2)/3;//(abs(micVolt - 530)) + 530;//   *(micVolt - 55))/512;
